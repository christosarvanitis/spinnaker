...
class DockerRegistryHelmLookupController {
  @Autowired
  ...
  List<String> getTags(@RequestParam('account') String account, @RequestParam('repository') String repository) {
    ...
    return DockerRegistryProviderUtils.getAllMatchingKeyPattern(
      ...
      Keys.getHelmTaggedImageKey(account, repository, "*")
    ).sort { a, b ->
  ...
  List<Map> find(LookupOptions lookupOptions) {
    ...
    if (lookupOptions.q) {
      def key = Keys.getHelmImageIdKey("*${lookupOptions.q}*")
      ...
      def key = Keys.getHelmTaggedImageKey(account, image, tag)
